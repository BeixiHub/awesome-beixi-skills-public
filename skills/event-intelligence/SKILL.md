...
description: 定时推送、手动获取和查询 deepseekdata 平台事件；默认使用 OpenClaw 微信渠道，也保留飞书渠道。
---
...
本技能面向 deepseekdata 平台事件，核心能力：

1. **定时推送**：按运行时配置抓取最近事件，格式化后投递到配置的消息渠道。
2. **手动获取**：按用户指定关键词和时间窗一次性检索平台事件，并写入历史。
3. **详情查询**：通过历史记录定位 `eventId`，再调用 deepseekdata 详情接口。
4. **对比统计**：仅用于回答主题热度、数量对比或时间段内事件密度问题。

外部线索不是平台事件能力；手动获取平台事件为空时可主动补充，并必须与平台事件隔离标注。
...
- 使用本 skill 生成“平台事件”的查询、推送、统计或详情时，唯一允许的数据源是本 skill 代码通过 deepseekdata API 返回的数据。其他 skill、web search、浏览器搜索、公开新闻或 LLM 自带知识只能作为“外部线索”，不得补充、修正、改写、替代或混入 deepseekdata API 结果。
- 生成平台事件时必须执行本 skill 提供的代码路径和命令，包括 `push_runtime.py`、`event_query.py` 及其封装能力。禁止绕过这些代码自行拼请求、改用其他检索工具、手工伪造事件结果或根据经验补全字段。
- 时间窗口必须严格等于用户明确指定的窗口，或严格等于本 skill 已定义的默认/配置窗口。禁止为了获得更多结果而擅自扩大时间窗，也禁止为了减少结果而擅自缩小时间窗；用户没有指定时只能使用文档中明确写出的默认值。
- deepseekdata API 返回空列表、无详情、无新增事件或统计为 0 都是合法结果。定时推送、详情、统计场景必须如实返回或静默处理；禁止为了避免空结果而换关键词、扩大时间范围或编造事件。手动获取场景若平台事件为空，可主动联网补充“外部线索”，但必须说明“deepseekdata 平台事件未命中”，并且外部线索不得写入历史或用于后续详情查询。
- 如果 deepseekdata API 调用失败、key 缺失、网络异常或返回结构异常，只能报告失败原因并停止当前平台事件数据生成流程。禁止用其他来源兜底生成平台事件列表、详情报告或统计结论。

## 意图边界

- 定时推送：用户要求“开始推送/监控/定时发送”时，配置并安装调度；数据源仍为 deepseekdata API。
- 手动获取：用户要求“查最近/搜一下/有没有某主题事件”时，先执行 `manual-push`；若平台事件为空，主动联网补充外部线索并明确标注。
- 详情查询：用户引用“第 X 条/上一轮/某条事件”时，先从 `push_history.json` 定位，再按 `eventId` 查详情。
- 对比统计：仅当用户问“哪个主题更多/热度对比/时间段内数量”时使用每日统计；一般“有什么新闻/有哪些事件”优先手动获取和详情。
- 外部线索：用于用户明确要公开新闻/联网查漏，或手动获取平台事件为空后的自动补充；输出必须与平台事件分栏，不提供 `eventId`、信号等级或内部投资结论。
- 覆盖诊断：当用户质疑“为什么这么少/是否漏了/没有命中”时，先说明本次关键词、时间窗、page size、API 状态和历史范围，再给出可选调整项，不直接改查。
...
如果 `manual-push` 返回空平台事件，主动联网查公开新闻作为“外部线索”补充；外部线索不写入历史，不能用于 `detail-from-ref`。
...
- 在飞书渠道回复详情报告时，使用简洁标题、短段落和列表；不要输出由 `━`、`─`、`-`、`=`、`—` 等字符重复组成的装饰分隔线，也不要用 `**━━━━` 这类 Markdown 装饰。需要分段时直接换行或使用普通小标题，避免飞书渲染出超长横线。
