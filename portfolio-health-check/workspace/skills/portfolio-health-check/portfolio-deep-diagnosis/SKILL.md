...
1. 数一下 `holdings` 数组里有几只股票(现金行不算)，记作 `stock_count`
2. 按**每只股票 3 分钟 + 固定 30 分钟下限**估算总耗时：
   `预计时间 = max(stock_count × 3, 30) 分钟`
3. 用这个估算告诉用户：

```
收到，正在进行深度诊断分析。根据您的 {stock_count} 只股票，这一步预计需要约 {max(stock_count × 3, 30)} 分钟，我会持续等待结果返回，请稍候...
```

示例：
- 2 只股票 → `max(6, 30)` = "这一步预计需要约 30 分钟"
- 10 只股票 → `max(30, 30)` = "这一步预计需要约 30 分钟"
- 15 只股票 → `max(45, 30)` = "这一步预计需要约 45 分钟"

**为什么有 30 分钟下限**：Phase-2 里 LLM 舆情分析、QVeris 初始化、PDF 渲染都有固定开销，不随股票数缩水。实测 2 股票实际耗时约 11 分钟，但整个等待要留出足够裕度给 QVeris / OpenRouter 偶发慢响应。

这个估算与 `call_remote_phase_api.py` 内部的客户端 timeout 完全对齐（`max(stock_count × 180 秒, 1800 秒)`）——超过这个时间客户端会主动中断，不会无限等待。
...
| 错误 | 用户提示语 | 后续动作 |
|------|----------|---------|
| 连接失败（URLError） | "分析服务两次连接都失败，可能是网络或服务器问题。建议稍后再试。" | 客户端已自动重试 1 次（同一幂等 key，不会重复扣费）；仍失败即终止 |
| HTTP 4xx 且 `msg` 含"积分不足" | "您的开放平台积分不足以完成本次诊断，请先充值。" | 不重试 |
...
补充说明：
- `call_remote_phase_api.py` 的客户端超时按 `max(stock_count × 3 分钟, 30 分钟)` 动态计算（最低 30 分钟保底）。40+ 只股票上限会超过 82 服务器 nginx 的 7200s 读超时,此时 nginx 会先中断并返回 504,属预期(现实中个人组合很难到 40 只股票)。
- 网络抖动时客户端会用同一个 `X-Idempotency-Key` 自动重试 1 次；借助 Java 的 `openapi_consume_record` 幂等表，重试不会重复扣分。
- PDF 渲染时间通常在估算范围内；接近上限属于正常，不要因为等待时间长就中止命令。
