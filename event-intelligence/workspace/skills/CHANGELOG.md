...
## [2026-04-10 17:30] 架构重构 + Skill 优化 + Web Search 仓位估算
...
## [2026-04-10 17:45] Web Search 自动估算持仓占比

### portfolio-quick-diagnosis/SKILL.md

- **新增仓位补全逻辑**：用户只给股票名没给仓位时，自动通过 web search 查询最新市值，按市值比例估算占比，生成确认表供用户核实
- 给了股数的情况同理：搜索股价 → 计算市值 → 归一化占比
- 确认表中标注数据来源（"占比为按市值估算，请核实"）
- 减少对用户的追问次数，只在搜不到可靠数据或比例不足 100% 时才询问
