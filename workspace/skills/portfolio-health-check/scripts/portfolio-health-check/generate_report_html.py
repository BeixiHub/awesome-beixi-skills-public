...
def cn_tol(t): return {"conservative":"保守","moderate":"稳健","aggressive":"积极","very_aggressive":"激进"}.get(t,t)
def cn_sty(s): return {"timing_rotation":"择时轮动","full_rotation":"满仓轮动","constant_mix":"恒定比例","dca":"定投渐进","core_satellite":"核心+卫星"}.get(s,s)
def cn_sec(s): return {"banking":"银行","broad-equity":"宽基指数","consumer-staples":"消费","new-energy":"新能源"}.get(s,s)
def kc(m,v):
    if v is None: return "#64748B"
    if m=="sharpe": return "#12B76A" if v>0.5 else "#F04438" if v<0 else "#F79009"
    if m=="dd": return "#12B76A" if v<0.10 else "#F04438" if v>0.20 else "#F79009"
    if m=="vol": return "#12B76A" if v<0.15 else "#F04438" if v>0.25 else "#F79009"
    return "#0F172A"

def _corr_bg(v):
    a = 0.15 + 0.85*abs(v)
    if v >= 0.5: return f"rgba(192,57,43,{a:.2f})"
    if v >= 0.2: return f"rgba(230,126,34,{a:.2f})"
...
def _radar_svg(vals, labels, sz=260):
    ...
    def p(a,rad): return cx+rad*math.cos(a), cy-rad*math.sin(a)
    s = [f'<svg viewBox="0 0 {sz} {sz}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:{sz}px">']
    for f in [.25,.5,.75,1]:
        pts=" ".join(f"{p(a,r*f)[0]:.1f},{p(a,r*f)[1]:.1f}" for a in angs)
        s.append(f'<polygon points="{pts}" fill="none" stroke="#E2E8F0" stroke-width=".5"/>')
    for a in angs:
        x2,y2=p(a,r); s.append(f'<line x1="{cx}" y1="{cy}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="#E2E8F0" stroke-width=".5"/>')
    pts=" ".join(f"{p(angs[i],r*(vals[i]+1)/2)[0]:.1f},{p(angs[i],r*(vals[i]+1)/2)[1]:.1f}" for i in range(n))
    s.append(f'<polygon points="{pts}" fill="rgba(47,120,173,.15)" stroke="#2F78AD" stroke-width="2"/>')
    for i in range(n):
        mp=r*(vals[i]+1)/2; x,y=p(angs[i],mp)
        s.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="4" fill="#2F78AD"/>')
        lx,ly=p(angs[i],r+18)
        s.append(f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="middle" font-size="10" fill="#334155" font-family="Microsoft YaHei,sans-serif">{labels[i]}</text>')
        vx,vy=p(angs[i],mp+14)
        s.append(f'<text x="{vx:.1f}" y="{vy:.1f}" text-anchor="middle" font-size="8" fill="#2F78AD" font-weight="600">{vals[i]:+.2f}</text>')
    s.append('</svg>')
    return "\n".join(s)


def generate_html(result, output_path):
    d=result["data"]; m=d["metadata"]; pm=d["risk_metrics"]["portfolio"]
    hs=d["risk_metrics"]["holdings"]; cm=d["correlation_matrix"]
    fe=d["factor_exposure"]; rc=d["risk_contribution"]; co=d["concentration"]
    bm=d["benchmark"]; se=d["sector_exposure"]; lq=d["liquidity"]; fl=d["risk_flags"]
    n=len(cm["labels"]); sl=[l.split(".")[0] for l in cm["labels"]]
    ...
    for h in hs:
        ...
        rec=str(h["max_dd_recovery_days"])+"天恢复" if h["max_dd_recovery_days"] is not None else "尚未恢复"
    ...
    for h in sorted(rc["by_holding"],key=lambda x:-x["pct_risk_contribution"]):
        w=h["pct_risk_contribution"]/mx_rc*100; name=nm.get(h["code"],h["code"])
    ...
    for h in lq["holdings"]:
        name=nm.get(h["code"],h["code"]); days=h.get("liquidation_days")
        ...
        if not has_pmv:
            ...
            cls="fm"; level="无数据"; desc=f"{name} 无成交数据，无法评估流动性。"; val_line=""
        elif days>5:
            cls="fh"; level="流动性紧张"
