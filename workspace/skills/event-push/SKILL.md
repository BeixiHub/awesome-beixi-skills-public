...
▸ 分支逻辑:
  （从 investmentLogic 中提取各分支链路，如：
    · AI多模态布局: 持股PixVerse → 影视大模型领先 → 商业化落地 → 股权增值
    · 估值重估逻辑: PB低于行业 → AI资产未充分定价 → 重估空间明确
  ）

▸ 多空博弈:
  （从 investmentLogic 中提取红蓝方胜率，如：
...
（数据来自 investmentTargetsSummary 数组，每行一个标的。必须使用 Markdown 表格格式输出，不要用空格对齐。）
...
- 使用全角分隔线 `━` 和 `─` 区分报告边界和板块边界
- 板块标题用中文序号（一、二、三...），便于用户引用（如"第三部分再详细说说"）
- 数值型字段（置信度、成功率）转为百分比显示
- `investmentLogic` 是一段长文本，需要智能拆分为核心逻辑、分支链路、多空博弈三个子段落
- `overallReasoningChain` 是 JSON 字符串格式的数组，必须先解析再格式化输出
